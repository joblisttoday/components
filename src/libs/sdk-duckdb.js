...
export class JoblistDuckDBSDK {
  constructor(baseParquetUrl = "https://workers.joblist.today", options = {}) {
	...
	async initialize() {
		...
		try {
			...
			console.log("FTS extension not available, falling back to LIKE search:", e.message);
			this.ftsEnabled = false;
		}
  ...
  async getStats() {
    ...
    try {
      ...
      console.log("Jobs parquet file might not exist at:", this.parquetUrl("jobs"));
    }
  ...
  async searchCompanies(query = "") {
    ...
    if (this.ftsEnabled) {
      try {
        ...
        await this.createFTSIndex(`'${vname}'`, "companies_fts_idx", ["id", "tags"]);
        ...
        console.log("FTS search failed, falling back to LIKE search:", e.message);
      }
    }
  ...
  async searchJobs(query = "") {
    ...
    if (this.ftsEnabled) {
      try {
        ...
        await this.createFTSIndex(`'${vname}'`, "jobs_fts_idx", ["name", "company_id", "company_title", "location"]);
        ...
        console.log("FTS search for jobs failed, falling back to LIKE search:", e.message);
      }
    }
