...
export default class MapList extends HTMLElement {
	...
	get origin() {
		return this.getAttribute('origin') || 'https://profiles.joblist.today/companies/{}'
	}
	...
	async insertStyles($context) {
		...
		try {
			await new Promise(async (resolve) => {
				...
				$leafletStyles.href = 'https://unpkg.com/leaflet@1.9.2/dist/leaflet.css'
				$leafletStyles.onload = resolve
				$leafletStyles.setAttribute('integrity', 'sha256-sA+zWATbFveLLNqWO2gtiw3HL/lh1giY/Inf1BJ0z14=')
	...
	renderMarkersLayer = (markers) => {
		...
		if (
			this.layerGroupMarkers
			&& this.map.hasLayer(this.layerGroupMarkers)
		) {
		...
		this.layerGroupMarkers = this.leaflet.featureGroup($markers).addTo(this.map)
...
if (!customElements.get('joblist-map-list')) {
	customElements.define('joblist-map-list', MapList);
}
